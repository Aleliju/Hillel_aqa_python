...
for car, data in car_data.items():
    if data[1] >= search_criteria[0] and data[2] >= search_criteria[1] and data[4] <= search_criteria[2]:
        sort_car_by_price.append((car, data))
        print("Sorted car by criteria: ", car, data)

sort_car_by_price.sort(key=lambda x: x[1][4])
print("Sorted car by price: ", sort_car_by_price)

for car, data in sort_car_by_price[:5]:
    print(f"{car}: Color: {data[0]}, Year: {data[1]}, Engine: {data[2]}L, Type: {data[3]}, Price: ${data[4]}")
